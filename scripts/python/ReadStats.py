...
class Statistics:
    ...
    def __init__(self,data_path):
        ...
        for n in range(0,self.t_len):
            self.L_Oz[n,:]    = (self.Eps[n,:]/self.rS_y[n,:]**3)**0.5
...
class Pdfs:
    ...
    def __init__(self,data_path_pdf_list,data_path_ydat,sizeofdata=4,etype='<',dtype='f',dimension=1):
        ...
        for n in range(len(data_path_pdf_list)):
            ...
            fin.close()
        pdf = pdf.reshape((len(data_path_pdf_list),self.ny + 1,self.nb + 2))
        self.pdf = pdf
        self.pdf_timeavg = np.sum(self.pdf,axis=0) / len(data_path_pdf_list)
